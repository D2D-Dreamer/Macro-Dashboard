...
st.markdown("Track the health of the US economy. Designed for executive tracking and presentations.")
st.divider()
...
for tab, cat_name in zip(tabs, tab_names):
    with tab:
        ...
        if view_type == "Absolute Values":
            fig = px.line(df_filtered, x=df_filtered.index, y='Value', title=f"{selected_metric} (Absolute)", template="plotly_white", color_discrete_sequence=[m_color])
            fig.update_layout(yaxis_title="Index / Rate / Value", xaxis_title="")
        else:
            fig = px.bar(df_filtered, x=df_filtered.index, y='YoY %', title=f"{selected_metric} (Year-over-Year % Change)", template="plotly_white", color_discrete_sequence=[m_color])
        ...
        summary_formatted = summary_df.style.format({'Value': "{:,.2f}", 'MoM %': "{:+.2f}%", 'YoY %': "{:+.2f}%"}).map(
            lambda val: 'color: green' if pd.notnull(val) and val > 0 else ('color: red' if pd.notnull(val) and val < 0 else 'color: gray'), subset=['MoM %', 'YoY %'])
